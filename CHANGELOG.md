...
- 通用模型阶段的重试携带最近一次无效输出与明确的校验错误，避免错误分类等格式问题在无反馈的重试中重复出现。
- 中间的网络超时不覆盖有效诊断；回归测试验证修正审稿结构后原始批评和 `invalid` 证据判定仍然保留。

## 2026-09-22 — 指标采集与日志显示分离

- 真实稳健均值实验发现长指标行被日志截尾截掉前缀，导致错误触发代码修复；现在从完整标准输出独立采集数值证据。
- 正式入口只使用自身最后一个指标对象，不再混入 smoke test 或拼接多个对象的旧字段。
- 指标行解析设置 1 MiB 上限；超限和损坏的显式指标对象明确失败，大型原始数组不进入 checkpoint 指标映射。
- 修复提示携带实际采集指标，并明确主指标必须为数值；200 项 Python 测试通过。

## 2026-09-22 — 长响应流式接收

- 实测兼容接口支持 SSE，内置配置启用流式响应；首段及后续约每 10 秒记录接收进度，长代码生成不再必须等待完整响应才看到输出。
- 校验结束原因和流结束标记，拒绝断流的部分源码、空响应、长度截断及内容过滤；兼容供应商返回普通 JSON。
- 保留事件/响应大小限制和耗时检查；增加本地 HTTP 分片传输、Unicode、断流和错误响应测试。
...
- 文件 manifest 增加依赖关系与行为契约，生成前校验循环、未知依赖及大小写/分隔符重复路径。
- 依赖文件先生成、实验入口最后生成；后续请求携带已有源码、方法不变量与先前审稿反馈。
- 简单实验使用较少的完整模块，避免强制拆成 6–10 个文件；多步 smoke test 关注方法和基线是否实际接收观测。
- 回归测试覆盖中断后恢复源码上下文并真实执行调用链；全部 182 项 Python 测试通过。有效科研结论仍以各会话验收报告为准。

## 2026-09-22 — 可追溯的方法审计复核

- 方法审计的阻塞意见增加独立复核，要求候选逐字引文与数值或执行依据；不确定意见保留候选等待核验。
- 首次方法设计同样执行审计，并传递用户原始研究约束；历史已完成方法保留兼容读取。
- 区分审计格式失败、复核未定和确认的候选错误，超时后只恢复尚未完成的子步骤。
- 拒绝前归档候选与完整审计，失败 checkpoint 同时保留原文，避免只剩错误摘要。
- 通过记录绑定候选哈希，离线验收检查审计后方法变更；CLI/TUI 显示复核进度。
- 增加固定模型响应的复核恢复对照，验证恢复 1 次请求与重启 3 次请求得到相同产物；不据此声称提升模型判断准确率。

## 2026-09-21 — 可执行验收与原创性边界

- 增加确定性的 session 验收报告，校验 checkpoint、产物哈希、执行指标、论文追溯和审稿结果。
- 在 OpenCode Evidence 页面显示验收结论，并检测 checkpoint 变化后的过期报告。
- 增加恢复机制与证据门的离线对照实验，以及有边界的开源项目能力对照。
- 兼容 Windows PowerShell 产生的 UTF-8 BOM 服务请求。
- 对 Windows 索引器或杀毒软件造成的短暂原子替换占用进行有限重试。
- 增加审稿驱动的 `revise` 闭环，从方法阶段重跑并在 TUI 显示修订轮次。
- 修复科研 JSON 中单反斜杠 LaTeX 命令导致完整模型响应无法解析的问题。
- 为 Coder 增加逐文件原子草稿，供应商超时后继续缺失文件而非重做整个项目。
- 为 Writer 增加标题、摘要与逐章节原子草稿，响应截断后从未完成章节继续，并在 CLI/TUI 显示进度。
- 保留嵌套实验指标并加入跨课题的提出方法/基线/差值/样本数契约与算术校验。
- 扩大评审源码证据预算并显式标记上下文裁剪，防止把提示裁剪误报为源码不完整。
- 为审稿修订增加方法不变量与独立一致性审计，在编码前拦截变量方向、投影符号和伪闭式梯度矛盾。
- 对 Python traceback 使用最深项目帧做确定性单文件修复，避免明确错误仍触发整项目重写。
- 持久化已验证的方法候选，使一致性审计超时后只重试审计，并在 CLI/TUI 投影等待状态。
- 状态接口压缩超长阶段/任务错误摘要，完整诊断继续保存在 checkpoint 与日志端点。
- 在接受 manifest 和恢复编码草稿时校验入口文件属于清单，避免完整生成后才发现入口缺失并循环复用坏草稿。
- 将无法确定定位时的模型修复计划限制为最多两个文件，防止单点错误扩大为整项目重写。
- 将指标缺行、缺键、约束与差值错误确定性定位到入口文件，并明确校正 `HARNESS_METRICS=` 协议前缀。
- 将发表建议与证据有效性拆分，允许可信负结果通过证据验收并拦截 validity 缺陷。
