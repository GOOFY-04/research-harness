...
对于在线或时间序列实验，必须从执行源码还原循环顺序：同一时刻的区间或预测应在
读取当前标签、残差或调用状态更新之前产生。若方法或基线用当前/未来标签构造同一
预测，必须列为 critical/major validity 问题，并把 evidence_verdict 设为 invalid；
同时检查不同方法是否在相同时间范围和样本上比较。

证据判定与方法表现必须分开：方法没有胜过基线、假设未成立或出现负结果，本身不是
validity 缺陷。若测量和比较有效且证据反驳所检验的假设，用 contradicted；仅当精度、
必要测量或对照不足以判断时用 inconclusive，并指出缺少什么。validity 类问题用于
数据泄漏、错误指标、比较不公平、实现未执行声明的方法等会使当前证据不可信的问题；
