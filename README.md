...
最新实测与已知缺口见 [2026-09-22 验证记录](docs/VALIDATION_20260922.md)，包含两个完整运行但未通过科研验收的可复跑案例。
完整实验配置现在要求[配对测量契约](docs/EXPERIMENT_CONTRACT.md)：运行前声明指标含义与比较范围，运行后由 harness 从归档数据重算配对统计。该检查不保证实验设计有效；真实审稿校准仍暴露出措辞和研究价值判断的问题。
...
内置配置启用 `llm.stream: true`，分段接收兼容接口的 SSE 响应，并在收到首段文本及后续约每 10 秒记录接收字符数。`timeout` 仍控制网络读取等待；流式接收在事件间检查阶段期限和 360 秒耗时上限。只有正常结束标记齐全才返回完整文本；断流、截断或过滤使请求失败，已验证草稿仍可恢复。仅返回普通 JSON 的服务也可使用；自定义接口可设置 `stream: false`。
...
通用模型阶段重试时会收到最近一次无效响应及其校验原因，中间发生网络超时也不会丢失这些反馈。修正结构不能用来删除科学批评或改变观测结果。
